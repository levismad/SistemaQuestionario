...
namespace SQ.Core.Services
{
    ...
    ///
    public class AuthSessionRequired :
      ServiceStack.ServiceInterface.RequestFilterAttribute, IHasRequestFilter
    {
        ...
        {
            var session = HttpContext.Current.Session.Contents["Usuario"] as MUsuario;
            DateTime requestDate;
            if (session != null && !DateTime.TryParse(SessionSignature.GetDate(req), out requestDate))
            {
                throw new SecurityException("You must log into the application to use the services.");
            }
            ...
            var signature = SessionSignature.GetSignature(req);
